![v](https://img.shields.io/badge/version-2.3.1-blue) ![v](https://img.shields.io/badge/updated-November%2029,%20%202023-green) ![Coverage](https://smokeshow.helpmanual.io/github/jakob1379/sweetviz/badge.svg) ![Tests](https://github.com/jakob1379/sweetviz/actions/workflows/test.yml/badge.svg)
...
# Installation
Sweetviz currently supports Python 3.10+ and Pandas 0.25.3+. Reports are output using the base "os" module, so custom environments such as Google Colab which require custom file operations are not yet supported, although I am looking into a solution. 
## Using pip
...
- **target_feat:** A string representing the name of the feature to be marked as "target". _Only BOOLEAN and NUMERICAL features can be targets for now._
- **feat_cfg:** A FeatureConfig object representing features to be skipped, or to be forced a certain type in the analysis. The arguments can either be a single string or list of strings. Parameters are `skip`, `force_cat`, `force_num` and `force_text`. The "force\_" arguments override the built-in type detection. They can be constructed as follows:
...
_\*If setting multiple values for `[general]` only include the `[General]` line once_.
...
Sweetviz uses a comprehensive test suite to ensure reliability and catch regressions. We follow a test-driven contribution model.
...
pytest --cov=sweetviz --cov-report=term --cov-report=html
...
When contributing bug fixes or new features:

1. **Write a test first** that demonstrates the issue or desired behavior
2. **Implement the fix/feature**
3. **Verify all tests pass**

See [CONTRIBUTING.md](CONTRIBUTING.md) for detailed guidelines on writing tests and contributing effectively.

## Continuous Integration

All pull requests run through GitHub Actions CI which tests:
